...
class Transport{
    ...
    private $login;
    private $password;
    var $charset_auto_detect = false;
    var $charset = "utf-8";
    var $https_address = "https://lcab.smsint.ru/";
    var $http_address = "http://lcab.smsint.ru/";
    var $https_method = "curl";
    var $use_https = true;

    function __construct($login,$password) {
        $this->login = $login;
        $this->password = $password;
        $this->https_method = function_exists('curl_init') ? 'curl' : 'file_get_contents';
    }
    ...
    function send($params = array(), $phones = array()){
        ...
        $someXML = "";
        if (isset($params["text"])) $params["text"] = htmlspecialchars($params["text"],null,$this->charset);
        foreach ($phones as $phone){
            if (is_array($phone)){
                if (isset($phone["number"])){
                    $someXML .= "<to number='".$phone['number']."'>";
                    if (isset($phone["text"])){
                        $someXML .= htmlspecialchars($phone["text"],null,$this->charset);
                    }
                    $someXML .= "</to>";
                }
            }
